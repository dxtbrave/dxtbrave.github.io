	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'studentId',title:'员工',width:200,
 		       	...
 		       	{field:'courseId',title:'培训课程',width:200,
 		        	formatter: function(value,row,index){
 						if (row.courseId){
	  ...
	  //设置编辑按钮
	    $("#edit").click(function(){
        	...
        	if(selectRows.length != 1){
            	$.messager.alert("芜湖", "请选择一条数据进行操作!", "warning");
            } else{
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectRow == null){
            	$.messager.alert("芜湖", "请选择数据进行删除!", "warning");
            } else{
            	var id = selectRow.id;
            	$.messager.confirm("芜湖", "将删除与培训课程相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							...
							success: function(msg){
								if(msg == "success"){
									$.messager.alert("芜湖","删除成功!","info");
									//刷新表格
									$("#dataList").datagrid("reload");
								}else if(msg == "not found"){
									$.messager.alert("芜湖","不存在该培训记录!","info");
								}else{
									$.messager.alert("芜湖","删除失败!","warning");
									return;
								}
							}
						});
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							$.messager.alert("芜湖","请检查你输入的数据!","warning");
							return;
						} else{
							$.ajax({
								type: "post",
								...
								success: function(msg){
									if(msg == "success"){
										$.messager.alert("芜湖","培训信息添加成功!","info");
										//关闭窗口
										...
										$.messager.alert("芜湖","该培训课程已经选满，不可再选!","warning");
										return;
									} else if(msg == "courseSelected"){
										$.messager.alert("芜湖","已经选了这门培训课程，不可再选!","warning");
										return;
									}else{
										$.messager.alert("芜湖","系统内部出错，请联系管理员!","warning");
										return;
									}
								}
							});
...
<body>
	...
	<div id="toolbar">
		...
		<div style="margin-top: 3px;">
			...
			培训课程：<input id="courseList" class="easyui-textbox" name="courseList" />
